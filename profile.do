...
quietly {
  ...
  *---------------------------------------------
    ...
    * Dialog box to select file manually
    capture window fopen path_and_run_do "Select the locator file for this project (`this_run_do'), expected to be inside any path/`this_repo'/" "Do Files (*.do)|*.do|All Files (*.*)|*.*" do
    ...
    if _rc == 0 {
      ...
      * If yes, attributes the path chosen by user to the clone, if not, exit
      if "`user_chosen_do'" == "`this_run_do'"  global clone "`user_chosen_path'"
        ...
        noi disp as error _newline "{phang}You selected $path_and_run_do as the locator file. This does not match what was expected (any path/`this_repo'/`this_run_do'). Code aborted.{p_end}"
        error 2222
      }
    }
  }
  ...
  *---------------------------------------------
    ...
    * Dialog box to select file manually
    capture window fopen path_and_run_do "Select the locator file for the data for this project (`this_run_do'), expected to be inside any path/`this_repo'/" "All Files (*.*)|*.*" do
    ...
    * If user clicked cancel without selecting a file or chose a file that is not a do, will run into error later
    if _rc == 0 {
      ...
      * Check if master do-file chosen by the user is master_run_do as expected
      * If yes, attributes the path chosen by user to the clone, if not, exit
      if "`user_chosen_do'" == "`this_run_do'"  global data_path "`user_chosen_path'"
      else {
        noi disp as error _newline "{phang}You selected $path_and_run_do as the locator file. This does not match what was expected (any path/`this_repo'/`this_run_do'). Code aborted.{p_end}"
        error 2222
      }
  ...
  *-----------------------------------------------------------------------------
  * 4) Download and install user written packages for the project
  *-----------------------------------------------------------------------------
  local commands keeporder
